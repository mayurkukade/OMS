...
const ClientReport = () => {
  ...
  const [chartData] = useState({
    series: [10000, 15000, 25000], // Estimate, Paid, Due amounts
    options: {
      chart: {
        type: 'pie',
        width: 350,
        ...
        events: {
          dataPointSelection: (event, chartContext, config) => {
            ...
            if (config.dataPointIndex === 0) {
              // Clicked on Estimate Amount
              alert(`Paid Amount: ${chartData.series[0]}`);
            } else if (config.dataPointIndex === 1) {
              // Clicked on Paid Amount
              alert(`Estimate Amount: ${chartData.series[1]}`);
            } else if (config.dataPointIndex === 2) {
              // Clicked on Due Amount
              alert(`Due Amount: ${chartData.series[2]}`);
            }
          },
        },
      },
      labels: ['Paid Amount', 'Due Amount', 'Estimate Amount'],
  ...
  const [chartData1] = useState({
    series: [10, 5, 5], // Estimate, Paid, Due amounts
    options: {
      chart: {
        type: 'pie',
        width: 350,
        ...
        events: {
          dataPointSelection: (event, chartContext, config) => {
            ...
            if (config.dataPointIndex === 0) {
              // Clicked on Estimate Amount
              alert(`Completed : ${chartData.series[0]}`);
            } else if (config.dataPointIndex === 1) {
              // Clicked on Paid Amount
              alert(`In Process: ${chartData.series[1]}`);
            } else if (config.dataPointIndex === 2) {
              // Clicked on Due Amount
              alert(`Started : ${chartData.series[2]}`);
            }
          },
        },
      },
      labels: ['Started', 'In Process', 'Completed'],
      colors: ['#6A5ACD', '#FF4500', '#FF8C00'],
      legend: {
        show: false,
      },
  ...
  return (
    <div>
      ...
      <div className="mt-8 space-y-4 ml-20">
        <Card>
          <CardHeader
            floated={false}
            shadow={false}
